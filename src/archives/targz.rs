...
impl TarGz {
    pub fn new() -> Self {
        let r_filename = r"^.*\.tar\.gz$";

        let r_file_cmd = r#"^.*tar archive.*gzip compressed data.*"#;
...
mod tests {
    ...
    use crate::archives::{archive::Archive, targz::TarGz, test_util::TestUtil};

    #[test]
    fn test_from_filename() {
        let archive = TarGz::new();
        assert_eq!(archive.from_filename("/tmp/hoge.tar.gz"), true);
        assert_eq!(archive.from_filename("/tmp/hoge.tar.bz2"), false);
        assert_eq!(archive.from_filename("/tmp/hoge.gz"), false);
        assert_eq!(archive.from_filename("/tmp/hogetar..gz"), false);
    }
    ...
    fn test_from_file_cmd() {
        assert_eq!(from_file_cmd("test_dir.tar.gz"), true);
        assert_eq!(from_file_cmd("test_dir.tar.bz2"), false);
        assert_eq!(from_file_cmd("test_dir.gz"), false);
    }
}
